...
WORKDIR /opt/braidcert/

    # write files generated during startup to /tmp
ENV RELX_OUT_FILE_PATH=/tmp \
    # braidcert specific env variables to act as defaults
    LOGGER_LEVEL=debug \
    SCHEDULERS=1 \
...
RUN touch ~/.profile && \
    printf "%s\n" "alias remshell='/opt/braidcert/bin/braidcert remote_console'" \
    >> ~/.profile

EXPOSE 80/tcp

ENTRYPOINT ["/opt/braidcert/bin/braidcert"]
CMD ["foreground"]
